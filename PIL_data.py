...
def read_mnc(filename="lena.mnc"):
  ...
  line = f.readline()
  while line:
    array_line = [int(i) for i in line.split()]
    big_array.append(array_line)
    line = f.readline()
  height = len(big_array)
  # denormalizing real and imag arrays
  real_array = np.array(big_array[:height/2])
  real_array = (real_array-b_real)/a_real
  imag_array = np.array(big_array[height/2:])
  imag_array = (imag_array-b_imag)/a_imag
  return real_array + 1j*imag_array,original_size
